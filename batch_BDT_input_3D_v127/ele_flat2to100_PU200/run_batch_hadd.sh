...
cd ${BATCH_DIR}
date
for filename in /afs/cern.ch/work/j/jheikkil/tools/ntuple-tools/triggerResults/tmp/histos_ele_flat2to100_PU200_eg_v127_*.root; do
    echo ${filename}
    xrdcp ${filename} .
done
hadd -j 10 -k histos_ele_flat2to100_PU200_eg_v127.root `ls histos_ele_flat2to100_PU200_eg_v127_*.root`
retVal=$?
if [ $retVal -ne 0 ]; then
    echo "****** Error: hadd failed!"
    exit $retVal
fi
rm histos_ele_flat2to100_PU200_eg_v127_*.root
