...
class Cluster3DGenMatchHybrid(BasePlotter):
    ...
    def plot3DMatch(self,
        ...
        if self.includeTracks:
            All3DClusters = match3DClusterToL1Tracks(All3DClusters, L1tracks)
        ...
        # REMOVE CLUSTERS THAT MATCHED TO GEN
        if self.saveNtuples:
            for match in allmatches.values():
                for k in match:
                    All3DClusters = All3DClusters.drop(k)
            for index, row in All3DClusters.iterrows():
                ntuple3DClNOMatch.fill(row)
        ...
        for idx, genParticle in genParticles.iterrows():
            if idx in best_match_indexes.keys():
                ...
                if self.saveNtuples:
                    ntuple3DClMatch.fill(matched3DCluster)
...
class Cluster3DHybrid(BasePlotter):
    ...
    def plot3D(self,
        ...
        # DECORATE THE 3D CLUSTERS WITH QUANTITIES OF MATCHED L1 TRACKS IN A DELTAR CONE
        if self.includeTracks:
            All3DClusters = match3DClusterToL1Tracks(trigger3DClusters, L1tracks)
